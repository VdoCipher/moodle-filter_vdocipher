    ...
    public function filter($text, array $options = array()) {
        ...
        return preg_replace_callback( '/\[vdo\s+([A-Za-z0-9\=\s\"\']+)\]/' , function($matches) {
            ...
            if (!empty(self::$watermark)) {
                global $USER;
                $vdoAnnotateCode = self::$watermark;
                if ( isset($USER) && !is_null($USER) ) {
                    $fullname = $USER->firstname . ' ' . $USER->middlename . ' ' . $USER->lastname;
                    $vdoAnnotateCode = str_replace('{name}', $fullname . ' ', $vdoAnnotateCode);
                    $vdoAnnotateCode = str_replace('{email}', $USER->email , $vdoAnnotateCode);
                    $vdoAnnotateCode = str_replace('{username}', $USER->username , $vdoAnnotateCode);
                    $vdoAnnotateCode = str_replace('{id}', $USER->id , $vdoAnnotateCode);
                }
                $vdoAnnotateCode = str_replace('{ip}', $_SERVER['REMOTE_ADDR'] , $vdoAnnotateCode);
                $vdoAnnotateCode = preg_replace_callback('/\{date\.([^\}]+)\}/', "eval_date" , $vdoAnnotateCode);
                if(!isset($attrs['no_annotate']))
                    $anno = array("annotate" => $vdoAnnotateCode);
            }
